...
urlpatterns = patterns(
  ...
  url(
    r'library/add/$', LibraryCreate.as_view(),
    name='library-add'),
  url(
    r'mylibrary/(?P<slug>[-\w]+)/book/add/$',
    BookCreate.as_view(),
    name='book-add'),
  url(
    r'^mylibrary/(?P<slug>[-\w]+)/$',
    LibraryDetailView.as_view(), name="library-detail"),
  url(
    r'^library-list/$', LibraryListView.as_view(),
    name='library-list'),
  url(
    r'^book-list/$', BookListView.as_view(),
    name='book-list'),
  url(
    r'^mylibrary/(?P<slug>[-\w]+)/manage-books/',
    ManageBooksFormView.as_view(),
    name='manage-books'),
  url(
    r'^mylibrary/(?P<lslug>[-\w]+)/book/(?P<slug>[-\w]+)/$',
    BookDetailView.as_view(), name="book-detail"),
  url(
    r'^mylibrary/(?P<lslug>[-\w]+)/book/(?P<slug>[-\w]+)/delete/$',
    BookDelete.as_view(), name='book-delete'),
  url(
    r'mylibrary/notification/$',
